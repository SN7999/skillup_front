...
const addChapter = async () => {
	const result = await getAddChapterAPI(addChapterForm.value.name, addChapterForm.value.introduction, classid, detialInfo.value.chapters.length + 1);
	if (result.data.code == 200) {
...
const hasVideos = computed(() => {
	if (selectedChapter.value) {
		...
		if (detialInfo.value.resources) {
			vid.value = detialInfo.value.resources.some(resource => resource.chapterId === selectedChapter.value && resource.type === '视频');
			if (vid.value == true) {
				return vid;
			}
		}
		if (unpassResource.value) {
			vid.value = unpassResource.value.some(resource => resource.chapterId === selectedChapter.value && resource.type === '视频');
			if (vid.value == true) {
				return vid;
			}
		}
		if (rejectResource.value) {
			vid.value = rejectResource.value.some(resource => resource.chapterId === selectedChapter.value && resource.type === '视频');
			if (vid.value == true) {
				return vid;
			}
		}
...
const hasTexts = computed(() => {
	if (selectedChapter.value) {
		const vid = ref(false);
		if (detialInfo.value.resources) {
			vid.value = detialInfo.value.resources.some(resource => resource.chapterId === selectedChapter.value && resource.type === '文本');
			if (vid.value == true) {
				return vid;
			}
		}
		if (unpassResource.value) {
			vid.value = unpassResource.value.some(resource => resource.chapterId === selectedChapter.value && resource.type === '文本');
			if (vid.value == true) {
				return vid;
			}
		}
		if (rejectResource.value) {
			vid.value = rejectResource.value.some(resource => resource.chapterId === selectedChapter.value && resource.type === '文本');
			if (vid.value == true) {
				return vid;
			}
		}
...
const uploadReApi = '/api3' + '/resource/upload?chapterid=' + selectedChapter.value;
...
const downloadUrls = {
	word:
		'https://skillup.oss-cn-beijing.aliyuncs.com/word%E6%A8%A1%E6%9D%BF.docx?Expires=1688807371&OSSAccessKeyId=TMP.3Kf15AoPr4ePCmCeQJuD49AebwX65ykYYb2YREE8TmzcfJ8CVxhv2vctQK9fXr7RcVeqz1XdC5D86bR7JE6LnWcDH4F16f&Signature=a7%2FUBY5iJ1ve9rlZzJ8I%2F9PQG5w%3D',
	excel:
			...
			<div class="main">
				...
				<div v-if="selectedMenuItem === 'classresource'">
					...
					<div v-if="!updateChapterVisible && !updateResourceVisible">
						<el-row class="announcement-header">
							<el-col :span="5"><h2>课程资料</h2></el-col>
							<el-col :span="4">
														...
														<div style="margin-top: 10px;" v-for="resource in detialInfo.resources" :key="resource.resourceId">
															<div v-if="resource.chapterId == chapter.chapter.chapterid && resource.status == '已通过'">
																<div
																	...
																	class="resource-box video"
																	@click="showVide(chapter.chapter.chapterid, resource)"
																>
					...
					<div v-if="updateChapterVisible">
						<el-row class="announcement-header">
							...
							<el-col :span="5"><h2>更新章节</h2></el-col>
							<el-col :span="4">
					...
					<div v-if="updateResourceVisible">
						<el-row class="announcement-header">
							...
							<el-col :span="5"><h2>添加资料</h2></el-col>
						</el-row>
						<div style="display: flex; flex-direction: column; align-items: center; margin-top: 5%;">
							<div style="margin-bottom: 20px; text-align: center;zoom: 120%;">
								<el-select v-model="selectedChapter" placeholder="请选择章节">
									<el-option
										v-for="chapter in detialInfo.chapters"
										:key="chapter.chapter.chapternum"
										:label="'第 ' + chapter.chapter.chapternum + ' 章：' + chapter.chapter.chaptername"
				...
				<div v-if="selectedMenuItem === 'markpaper' && markselected === 'questionpage'">
					...
					<el-table :data="questionList">
						...
						<el-table-column label="操作">
							<template #default="scope">
								<el-button type="primary" size="middle" @click="setmarkpage();setquestionshow(scope.row.question);getMarkLists(scope.row.questionid);clearAll();">批改</el-button>
							</template>
						</el-table-column>
					</el-table>
				</div>
				...
				<div v-if="selectedMenuItem === 'publishannounce'">
					<el-row class="announcement-header">
						<el-col :span="5"><h2>发布公告</h2></el-col>
						<el-col :span="12">
