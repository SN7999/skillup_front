...
const submitForm = async () => {
  for (const question of questions.value) {
    const answer = {
      ...
      studentanswer: answers.value[question.questionid]
    }
...
const startCountdown = () => {
  ...
  remainingTime.value = Math.floor(
    (backendTime.getTime() + interval - currentTime) / 1000
  ) // 设定倒计时时间（秒）
...
<template>
  <div class="container">
    ...
    <el-drawer v-model="drawerVisible" title="目录" direction="ltr" @mouseleave="hideDrawer" style="height: 100%;">
      <div v-for="(question, index) in questions" :key="question.questionid">
