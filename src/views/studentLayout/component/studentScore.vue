...
import {
  getScoreAPI,
  getSelfScoreAPI,
  getSearchScoreExamAPI,
  getSearchScoreClassAPI
} from '@/apis/studentScoreAPI'
...
<template>
  <div>
    ...
    <div class="searchBox" style="display: flex; align-items: center; margin-top: 10px;">
      <el-input v-model.trim="searchExamName" class="w-50 m-2" size="large" placeholder="请输入考试名称" :prefix-icon="Search" @keyup.enter="getSearchScoreExam" style="width: 25%; margin-right: 10px;" />
      <el-input v-model.trim="searchClassName" class="w-50 m-2" size="large" placeholder="请输入课程名称" :prefix-icon="Search" @keyup.enter="getSearchScoreClass" style="width: 25%; margin-right: 10px;" />
    ...
    <div class="pagination-container">
      <el-pagination @current-change="handlePageChange" :current-page="currentPage" :page-size="pageSize" :total="scoreList.length" layout="prev, pager, next"></el-pagination>
    </div>
  </div>
</template>
